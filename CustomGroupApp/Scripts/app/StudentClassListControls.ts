...
class StudentClassListControls {
...
class StudentSetListControls {
    ...
    createStudentSetContainer = (
        ...
        return this.createStudentSetGrid(name, gridElement.id, studentSets, isCoedSchool);;
    };
    ...
    createStudentSetGrid = (
        ...
        // Populate the grid
        var students: Array<{ 'names': string }> = [];
        Enumerable.From(studentSets).ForEach(x => students.push({'names' : x.studentList}));
        grid.dataSource.data(students);
        grid.refresh();
        grid.resize();
        return grid;
