class StudentMathsSkillsProfileViewModel extends StudentPortfolio
implements IStudentPortfolio {
    ...
    setAdditionalData = (callback: (status) => any) => {
        if (this.testFile.hasMathsQuestions) {
    ...
    setAdditionalProperties = (student: Student) => {
        this.scoreProfile = student.mathPerformance.scoreProfile;
        this.studentAnswers = this.scoreProfile.getStudentAnswers(this.testFile.mathsQuestions);
        this.strandSummary = this.scoreProfile.getStrandSummary(this.studentAnswers);
        this.strandLevels = Enumerable.From(this.testFile.mathsQuestions)
